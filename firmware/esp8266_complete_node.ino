/*
 * FOGNET-X Industrial IoT Node - ESP8266 Complete Version
 * =======================================================
...
// Note: ESP8266 has limited GPIOs, so we use D-pins on NodeMCU
...
// Gas Sensor (Analog)
#define MQ2_PIN A0          // ADC pin only on ESP8266

// PIR Motion
#define PIR_PIN D1          // GPIO 5

// Ultrasonic (Tank Level)
#define TRIG_PIN D2         // GPIO 4
#define ECHO_PIN D3         // GPIO 0

// Soil Moisture (Analog)
#define SOIL_MOISTURE_PIN A0  // Share ADC with MQ2 (use voltage divider or multiplexer)

// Sound Sensor
#define SOUND_PIN A0        // Share ADC pin

// Vibration Sensor
#define VIBRATION_PIN D5    // GPIO 14

// Light Sensor (LDR)
#define LIGHT_PIN A0        // Share ADC pin

// Water Flow Sensor
#define FLOW_PIN D6         // GPIO 12 (interrupt capable)

// Actuators
#define RELAY_FAN D7        // GPIO 13 - Fan control
#define RELAY_PUMP D8       // GPIO 15 - Pump/Valve control
#define SERVO_PIN D0        // GPIO 16 - Servo PWM (limited on ESP8266)
#define BUZZER_PIN D9       // GPIO 3 - Buzzer

// Status LEDs
#define LED_GREEN LED_BUILTIN  // Built-in LED (GPIO 16 inverted)
#define LED_YELLOW D10      // GPIO 1
#define LED_RED D12         // GPIO 13
...
// Device health
float batteryVoltage = 3.3;  // Simulated
int rssi = 0;
...
void setup() {
  ...
  Serial.println("\n");
  Serial.println("========================================");
  Serial.println("🏭 FOGNET-X ESP8266 Industrial Node");
  Serial.println("========================================");
  Serial.print("Device ID: ");
  Serial.println(DEVICE_ID);
  Serial.print("Chip Model: ESP8266 ");
...
void loop() {
  ...
  yield();  // ESP8266 needs yield() for background tasks
  delay(100);
}
...
void setup_wifi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.print(rssi);
    Serial.println(" dBm");
    digitalWrite(LED_GREEN, LOW);  // Built-in LED is inverted
  } else {
    wifiConnected = false;
...
void read_all_sensors() {
  ...
  // Soil moisture (Note: shares ADC with other sensors)
  soilMoisture = analogRead(SOIL_MOISTURE_PIN);
  
  // Sound level (shares ADC)
  soundLevel = analogRead(SOUND_PIN);
  
  // Vibration
  vibrationDetected = digitalRead(VIBRATION_PIN);
  
  // Light level (shares ADC)
...
void update_leds() {
  if (wifiConnected && mqttConnected) {
    digitalWrite(LED_GREEN, LOW);  // Inverted LED
  } else {
...
void publish_data() {
  ...
  // ESP8266 doesn't have internal temperature sensor like ESP32
